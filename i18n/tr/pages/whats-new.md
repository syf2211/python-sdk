...
* **Bir aracın içinde fırlatılan `MCPError` (v1'deki `McpError`) artık bir protokol hatasıdır.** Model onu asla görmez. Diğer her istisna hâlâ `is_error=True` bir sonuca dönüşür, ancak modele yalnızca bir `ToolError`'ın mesajı ulaşır: başka herhangi bir istisna artık `Error executing tool <name>` olarak okunur, traceback ise sunucu log'unuzda kalır. Ayrım **[Hataları ele alma](servers/handling-errors.md)** sayfasında.
